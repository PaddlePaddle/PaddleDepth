...
void Conv2d_LF_Cuda(paddle::Tensor x, paddle::Tensor y, paddle::Tensor z, size_t N1, size_t N2, size_t Ci, size_t Co, size_t B,
    ...
    PD_DISPATCH_FLOATING_TYPES(x.type(), "Conv2d_LF", ([&] {
        conv2d_kernel_lf<data_t> << < gridSize, blockSize >> > (
                x.data<data_t>(), y.data<data_t>(), z.data<data_t>(),
                        N1, N2, Ci, Co, B, K);
    }));
}
...
Conv2d_LB_Cuda(paddle::Tensor x, paddle::Tensor y, paddle::Tensor gx, paddle::Tensor gy, paddle::Tensor gz, size_t N1, size_t N2, size_t Ci,
    ...
    dim3 gridSize((N2 + blockSize.x - 1) / blockSize.x, (N1 + blockSize.y - 1) / blockSize.y,
                  (Co + blockSize.z - 1) / blockSize.z);
    PD_DISPATCH_FLOATING_TYPES(x.type(), "Conv2d_LB", ([&] {
        conv2d_kernel_lb<data_t> << < gridSize, blockSize >> > (
                x.data<data_t>(), y.data<data_t>(),
                        gx.data<data_t>(), gy.data<data_t>(), gz.data<data_t>(),
                        N1, N2, Ci, Co, B, K);
    }));
}
