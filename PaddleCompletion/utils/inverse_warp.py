...
class Intrinsics:
    def __init__(self, width, height, fu, fv, cu=0, cv=0):
        ...
        self.U = paddle.arange(start=0, end=width).expand(height, width).float()
        self.V = paddle.arange(start=0, end=height).expand(width,
                                                          height).t().float()
...
def image_to_pointcloud(depth, intrinsics):
    ...
    return paddle.concat((X, Y, depth), dim=1)
...
def pointcloud_to_image(pointcloud, intrinsics):
    ...
    pixel_coords = paddle.stack([U_proj_normalized, V_proj_normalized],
                               dim=2)  # [B, H*W, 2]
...
def transform_curr_to_near(pointcloud_curr, r_mat, t_vec, intrinsics):
    ...
    XYZ_ = paddle.bmm(r_mat, pointcloud_curr.view(batch_size, 3, -1))
    ...
    pointcloud_near = paddle.concat((X, Y, Z), dim=1)
