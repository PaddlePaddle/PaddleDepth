...
def merge_into_row(ele, pred):
    def preprocess_depth(x):
        y = np.squeeze(x.detach().cpu().numpy())
        return depth_colorize(y)

    # if is gray, transforms to rgb
    img_list = []
    if 'rgb' in ele:
        rgb = np.squeeze(ele['rgb'][0, ...].detach().cpu().numpy())
        rgb = np.transpose(rgb, (1, 2, 0))
        img_list.append(rgb)
    elif 'g' in ele:
        g = np.squeeze(ele['g'][0, ...].detach().cpu().numpy())
    ...
    # cv2.imwrite("/home/aistudio/test.png",img_merge)
