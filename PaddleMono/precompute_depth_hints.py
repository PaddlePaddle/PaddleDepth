...
class DepthHintDataset:
    ...
    Set up as a paddle dataset to make use of paddle DataLoader multithreading.
    """
    def __init__(self,
                 data_path,
                 filenames,
        ...
        self.K = \
            paddle.to_tensor(self.K, dtype='float32').unsqueeze(0).expand(self.data_size, -1, -1)
        self.invK = \
            paddle.to_tensor(self.invK, dtype='float32').unsqueeze(0).expand(self.data_size, -1, -1)

        self.baseline = 0.1  # the same baseline in datasets/mono_dataset.py
        self.T = paddle.eye(4, dtype='float32').unsqueeze(0)
    ...
    def compute_depths(self, base_image, lookup_image, reverse=False):
        ...
        disps = paddle.to_tensor(disps, dtype="float32")

        # convert disp to depth ignoring missing pixels
        depths = self.K[0, 0, 0] * self.baseline / (disps + 1e-7) * float((disps > 0))
    ...
    def __getitem__(self, index):
        ...
        base_image = paddle.to_tensor(base_image, dtype='float32').transpose(2, 0, 1).unsqueeze(0)\
            .expand(self.data_size, -1, -1, -1) / 255
        lookup_image = paddle.to_tensor(lookup_image, dtype='float32').transpose(2, 0, 1).unsqueeze(0)\
            .expand(self.data_size, -1, -1, -1) / 255
...
def run(opt):
    ...
    Makes use of paddle DataLoader multithreading.
     """
    ...
    for i, data in enumerate(dataloader):
        ...
        if data:
            ...
            best_index = paddle.argmin(losses, axis=0)
            best_depth = paddle.gather(data['depths'], axis=0, index=best_index).cpu().numpy()
...
def get_opts():
    ...
    parser.add_argument('--filenames',
                        ...
                        type=str,
                        default='splits/eigen/test_files.txt')
